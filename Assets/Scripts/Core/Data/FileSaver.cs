// <copyright file="FileSaver.cs" company="GreedyGuppyGames">
...
namespace Core.Data
{
    public abstract class FileSaver<T> : IDataSaver<T> where T : IDataStore
    {
        ...
        {
            this.m_Filename = GetFinalSaveFilename(filename);
        }
        ...
        {
            return new StreamWriter(new FileStream(this.m_Filename, FileMode.Create));
        }

        protected virtual StreamReader GetReadStream()
        {
            return new StreamReader(new FileStream(this.m_Filename, FileMode.Open));
        }
    }
}