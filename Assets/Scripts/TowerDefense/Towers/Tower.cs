// <copyright file="Tower.cs" company="GreedyGuppyGames">
...
namespace TowerDefense.Towers
{
    ...
    public class Tower : Targetable
    {
        ...
        {
            get { return this.currentLevel == this.levels.Length - 1; }
        }
        ...
        {
            get { return this.levels[this.currentLevel].towerGhostPrefab; }
        }
        ...
        {
            this.placementArea = targetArea;
            this.gridPosition = destination;

            if (targetArea != null)
            {
                this.transform.position = this.placementArea.GridToWorld(destination, this.dimensions);
                this.transform.rotation = this.placementArea.transform.rotation;
                targetArea.Occupy(destination, this.dimensions);
            }

            this.SetLevel(0);
            if (LevelManager.instanceExists)
            {
                LevelManager.instance.levelStateChanged += this.OnLevelStateChanged;
            }
        }
        ...
        {
            return this.GetSellLevel(this.currentLevel);
        }
        ...
        public int GetSellLevel(int level)
        {
            ...
            if (LevelManager.instance.levelState == LevelState.Building)
            {
                ...
                {
                    cost += this.levels[i].cost;
                }

                return cost;
            }

            return this.levels[this.currentLevel].sell;
        }
        ...
        {
            if (level < 0 || this.isAtMaxLevel || level >= this.levels.Length)
            {
                return false;
            }
        ...
        public override void Remove()
        {
            ...
            this.placementArea.Clear(this.gridPosition, this.dimensions);
        ...
        protected void SetLevel(int level)
        {
            ...
            {
                Destroy(this.currentTowerLevel.gameObject);
            }

            // instantiate the visual representation
            this.currentTowerLevel = Instantiate(this.levels[this.currentLevel], this.transform);

            // initialize TowerLevel
            this.currentTowerLevel.Initialize(this, this.enemyLayerMask, this.configuration.alignmentProvider);
            ...
            this.currentTowerLevel.SetAffectorState(initialise);
        }
        ...
        {
            this.configuration.SetMaxHealth(this.currentTowerLevel.maxHealth);

            if (this.currentLevel == 0)
            {
                this.configuration.SetHealth(this.currentTowerLevel.maxHealth);
            }
            else
            {
                int currentHealth = Mathf.FloorToInt(this.configuration.normalisedHealth * this.currentTowerLevel.maxHealth);
                this.configuration.SetHealth(currentHealth);
            }
        }
