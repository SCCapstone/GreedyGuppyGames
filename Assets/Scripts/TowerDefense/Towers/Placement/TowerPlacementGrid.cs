// <copyright file="TowerPlacementGrid.cs" company="GreedyGuppyGames">
...
namespace TowerDefense.Towers.Placement
{
    ...
    public class TowerPlacementGrid : MonoBehaviour, IPlacementArea
    {
        ...
        {
            Vector3 localLocation = this.transform.InverseTransformPoint(worldLocation);

            // Scale by inverse grid size
            localLocation *= this.m_InvGridSize;
        ...
        public TowerFitStatus Fits(IntVector2 gridPos, IntVector2 size)
        {
            ...
            if ((size.x > this.dimensions.x) || (size.y > this.dimensions.y))
            {
            ...
            if ((gridPos.x < 0) || (gridPos.y < 0) ||
                (extents.x > this.dimensions.x) || (extents.y > this.dimensions.y))
            {
                return TowerFitStatus.OutOfBounds;
            }
        ...
        public void Occupy(IntVector2 gridPos, IntVector2 size)
        {
            ...
            for (int y = gridPos.y; y < extents.y; y++)
            {
                for (int x = gridPos.x; x < extents.x; x++)
                {
                    this.m_AvailableCells[x, y] = true;

                    // If there's a placement tile, clear it
                    if (this.m_Tiles != null && this.m_Tiles[x, y] != null)
                    {
                        this.m_Tiles[x, y].SetState(PlacementTileState.Filled);
                    }
                }
            }
        }
        ...
        public void Clear(IntVector2 gridPos, IntVector2 size)
        {
            ...
            for (int y = gridPos.y; y < extents.y; y++)
            {
                for (int x = gridPos.x; x < extents.x; x++)
                {
                    this.m_AvailableCells[x, y] = false;

                    // If there's a placement tile, clear it
                    if (this.m_Tiles != null && this.m_Tiles[x, y] != null)
                    {
                        this.m_Tiles[x, y].SetState(PlacementTileState.Empty);
                    }
                }
            }
        }
        ...
        protected virtual void Awake()
        {
            ...
            this.m_AvailableCells = new bool[this.dimensions.x, this.dimensions.y];

            // Precalculate inverted grid size, to save a division every time we translate coords
            this.m_InvGridSize = 1 / this.gridSize;
        ...
        {
            var myCollider = this.GetComponent<BoxCollider>();
            Vector3 size = new Vector3(this.dimensions.x, 0, this.dimensions.y) * this.gridSize;
        ...
        protected void SetUpGrid()
        {
            ...
            if (tileToUse != null)
            {
                ...
                tilesParent.transform.parent = this.transform;
                tilesParent.transform.localPosition = Vector3.zero;
                tilesParent.transform.localRotation = Quaternion.identity;
                this.m_Tiles = new PlacementTile[this.dimensions.x, this.dimensions.y];

                for (int y = 0; y < this.dimensions.y; y++)
                {
                    for (int x = 0; x < this.dimensions.x; x++)
                    {
                        Vector3 targetPos = this.GridToWorld(new IntVector2(x, y), new IntVector2(1, 1));
                        targetPos.y += 0.01f;
        ...
        private void OnValidate()
        {
                ...
                this.dimensions.y <= 0)
            {
                ...
                this.dimensions = new IntVector2(Mathf.Max(this.dimensions.x, 1), Mathf.Max(this.dimensions.y, 1));
            }

            // Ensure collider is the correct size
            this.ResizeCollider();

            this.GetComponent<BoxCollider>().hideFlags = HideFlags.HideInInspector;
        }
        ...
        private void OnDrawGizmos()
        {
            ...
            for (int y = 0; y < this.dimensions.y; y++)
            {
                for (int x = 0; x < this.dimensions.x; x++)
                {
                    var position = new Vector3((x + 0.5f) * this.gridSize, 0, (y + 0.5f) * this.gridSize);
                    Gizmos.DrawWireCube(position, new Vector3(this.gridSize, 0, this.gridSize));
                }
            }
            ...
            Vector3 center = this.transform.TransformPoint(new Vector3(this.gridSize * this.dimensions.x * 0.5f,
                                                                  1,
                                                                  this.gridSize * this.dimensions.y * 0.5f));
