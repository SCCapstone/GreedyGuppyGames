// <copyright file="Triangulator.cs" company="GreedyGuppyGames">
...
namespace TowerDefense.MeshCreator
{
    public class Triangulator
    {
        ...
        public int[] Triangulate()
        {
            ...
            int numPoints = this.m_Points.Count;
            if (numPoints < 3)
            {
            ...
            for (int v = nv - 1; nv > 2;)
            {
                ...
                if (this.Snip(u, v, w, nv, workingIndices))
                {
                    int a, b, c, s, t;
                    a = workingIndices[u];
                    b = workingIndices[v];
        ...
        private float Area()
        {
            int n = this.m_Points.Count;
            float area = 0.0f;
            for (int p = n - 1, q = 0; q < n; p = q++)
            {
                Vector2 pval = this.m_Points[p];
                Vector2 qval = this.m_Points[q];
        ...
        {
            Vector2 a = this.m_Points[indices[u]];
            Vector2 b = this.m_Points[indices[v]];
            Vector2 c = this.m_Points[indices[w]];
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                Vector2 p = this.m_Points[indices[i]];
