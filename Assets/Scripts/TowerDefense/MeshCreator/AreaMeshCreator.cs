// <copyright file="AreaMeshCreator.cs" company="GreedyGuppyGames">
...
namespace TowerDefense.MeshCreator
{
    ...
    public class AreaMeshCreator : MonoBehaviour
    {
        ...
        public Transform pointsCenter
        {
            get
            {
                if (this.outSidePointsParent == null)
                {
                    var points = new GameObject("Points");
                    this.outSidePointsParent = points.transform;
                    this.outSidePointsParent.SetParent(this.transform, false);
                    this.outSidePointsParent.eulerAngles = new Vector3(90, 0, 0);
                }
#if UNITY_EDITOR
                this.outSidePointsParent.hideFlags = HideFlags.HideInHierarchy;
#endif
                return this.outSidePointsParent;
            }
        }
        ...
        public Transform[] pointsTransforms
        {
            get
            {
                Transform[] childern = new Transform[this.pointsCenter.childCount];
                int length = this.pointsCenter.childCount;
                for (int i = 0; i < length; i++)
                {
                    childern[i] = this.pointsCenter.GetChild(i);
                }
        ...
        {
            return this.GetChildrenPositions(this.pointsCenter);
        }
        ...
        {
            return this.transform.TransformPoint(this.meshObject.RandomPointInMesh());
        }
        ...
        public void ForcePointsFlat()
        {
            ...
            for (int i = 0; i < length; i++)
            {
                Transform t = this.pointsCenter.GetChild(i);
        ...
        private List<Vector3> GetChildrenPositions(Transform parent)
        {
        ...
        {
            int count = this.pointsCenter.childCount;
            for (int i = 0; i < count - 1; i++)
            {
                Vector3 from = this.pointsCenter.GetChild(i).position;
                Vector3 to = this.pointsCenter.GetChild(i + 1).position;
                Gizmos.DrawLine(from, to);
            }

            // last to first
            Vector3 last = this.pointsCenter.GetChild(count - 1).position;
            Vector3 first = this.pointsCenter.GetChild(0).position;
    ...
    public class MeshObject
    {
        ...
        public MeshObject(List<Triangle> triangles)
        {
            ...
            this.completeArea = this.triangles.Sum(x => x.area);
        }
        ...
        {
            Triangle randomTriangle = this.triangles.WeightedSelection(this.completeArea, t => t.area);
