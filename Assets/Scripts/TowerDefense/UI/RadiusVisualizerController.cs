// <copyright file="RadiusVisualizerController.cs" company="GreedyGuppyGames">
...
namespace TowerDefense.UI
{
    public class RadiusVisualizerController : MonoBehaviour
    {
        ...
        private readonly List<GameObject> m_RadiusVisualizers = new List<GameObject>();
        ...
        public void SetupRadiusVisualizers(Tower tower, Transform ghost = null)
        {
            ...
            {
                if (this.m_RadiusVisualizers.Count < i + 1)
                {
                    this.m_RadiusVisualizers.Add(Instantiate(this.radiusVisualizerPrefab));
                }

                ITowerRadiusProvider provider = providers[i];

                GameObject radiusVisualizer = this.m_RadiusVisualizers[i];
                radiusVisualizer.SetActive(true);
                radiusVisualizer.transform.SetParent(ghost == null ? tower.transform : ghost);
                radiusVisualizer.transform.localPosition = new Vector3(0, this.radiusVisualizerHeight, 0);
                radiusVisualizer.transform.localScale = Vector3.one * provider.effectRadius * 2.0f;
                radiusVisualizer.transform.localRotation = new Quaternion { eulerAngles = this.localEuler };
        ...
        {
            foreach (GameObject radiusVisualizer in this.m_RadiusVisualizers)
            {
                radiusVisualizer.transform.parent = this.transform;
